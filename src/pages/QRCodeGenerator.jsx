  ...
  const downloadQRCode = async (format = 'png') => {
    ...
    try {
      ...
      if (format === 'png') {
        ...
        // Calculate QR code size to fit nicely on page (larger size)
        const maxImageSize = Math.min(pageWidth - 40, pageHeight - 40, 150) // Larger QR code
        const imageX = (pageWidth - maxImageSize) / 2
        const imageY = (pageHeight - maxImageSize) / 2

        // Add only the QR code image (centered on page)
